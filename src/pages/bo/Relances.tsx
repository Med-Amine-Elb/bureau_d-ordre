  ...
  return (
    <div className="w-full max-w-7xl mx-auto pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1 ml-11">Centralisez les rappels et suivez les dossiers bloqués.</p>
      </div>

      {/* Stats Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-2xl border border-slate-200 soft-shadow">
          <h3 className="text-sm font-semibold text-slate-500 mb-2">Fournisseurs à contacter</h3>
          <div className="text-3xl font-bold text-slate-900">{fournisseursDossiers.length}</div>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-slate-200 soft-shadow">
          <h3 className="text-sm font-semibold text-slate-500 mb-2">Prescripteurs en retard</h3>
          <div className="text-3xl font-bold text-slate-900">{prescripteursDossiers.length}</div>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-slate-200 soft-shadow">
          <h3 className="text-sm font-semibold text-slate-500 mb-2">Relances effectuées ce mois</h3>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 soft-shadow overflow-hidden">
        ...
        <div className="flex items-center border-b border-slate-200 bg-slate-50/50">
          <button 
            ...
            className={`flex-1 md:flex-none px-6 py-4 text-sm font-semibold transition-colors border-b-2 ${
              activeTab === "fournisseurs" ? "border-blue-600 text-blue-600 bg-white" : "border-transparent text-slate-500 hover:text-slate-700"
            }`}
          >
          ...
          <button 
            ...
            className={`flex-1 md:flex-none px-6 py-4 text-sm font-semibold transition-colors border-b-2 ${
              activeTab === "prescripteurs" ? "border-blue-600 text-blue-600 bg-white" : "border-transparent text-slate-500 hover:text-slate-700"
            }`}
          >
        ...
        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-white">
          <div className="relative w-[300px]">
            ...
            <input type="text" placeholder="Rechercher..." className="w-full pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm text-black placeholder:text-black/60 bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
          </div>
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-100">
                ...
                <th className="px-6 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Cible</th>
                <th className="px-6 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Jours en attente</th>
                ...
                currentList.map(dossier => (
                  <tr key={dossier.new_dossierid} className="border-b border-slate-50 hover:bg-slate-50/50">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-slate-500">{dossier.new_type_document === 100 ? 'Facture' : 'Autre'}</div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => navigate(`/bo/dossiers/${dossier.new_dossierid}`)} className="inline-flex items-center justify-center w-8 h-8 rounded-lg text-slate-400 hover:bg-slate-100 mr-2">
                        <Eye className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleOpenRelance(dossier)} className="inline-flex items-center gap-2 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg text-sm font-semibold transition-colors">
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center">
              <h2 className="text-xl font-bold text-slate-800">Nouvelle Relance</h2>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="text-sm font-semibold text-slate-700 block mb-2">Canal de relance</label>
                <div className="flex gap-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" checked={relanceType === "email"} onChange={() => setRelanceType("email")} className="text-blue-600" />
                    <span className="text-sm flex items-center gap-1.5"><Mail className="w-4 h-4 text-slate-400" /> Email</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" checked={relanceType === "tel"} onChange={() => setRelanceType("tel")} className="text-blue-600" />
                    <span className="text-sm flex items-center gap-1.5"><PhoneCall className="w-4 h-4 text-slate-400" /> Téléphone</span>
                  </label>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none"
                />
              </div>
            </div>
